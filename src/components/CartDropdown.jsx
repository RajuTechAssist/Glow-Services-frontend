...
const CartDropdown = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-96 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 z-50">
      {/* Header */}
      ...
      <div className="max-h-96 overflow-y-auto custom-scrollbar">
        {items.length === 0 ? (
          ...
          <div className="p-4 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-xs capitalize">{item.category} • {item.duration}</p>
                  <p className="text-pink-600 dark:text-pink-400 font-bold text-sm">₹{item.price}</p>
                </div>
      ...
      {/* Footer */}
      {items.length > 0 && (
        <div className="border-t border-gray-100 dark:border-gray-700 p-4 space-y-4 bg-gray-50 dark:bg-gray-800">
          <div className="flex items-center justify-between">
            <span className="text-lg font-bold text-gray-900 dark:text-white">Total:</span>
            <span className="text-xl font-bold text-pink-600 dark:text-pink-400">₹{getTotalPrice()}</span>
          </div>
          
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={clearCart}
              className="py-3 px-4 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200"
            >
              Clear Cart
            </button>
            <Link
